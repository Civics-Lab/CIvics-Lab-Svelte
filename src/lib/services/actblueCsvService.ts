...
export async function pollCsvStatus(
  ...
  while (attempts < maxAttempts) {
    const status = await checkCsvStatus(csvId, workspaceId, token);
...
export async function requestAndImportCsv(
  ...
  const { csvId } = await requestCsvExport(workspaceId, csvType, dateRangeStart, dateRangeEnd, token);

  // Step 2: Wait for CSV to be ready
  onProgress?.('waiting');
  const status = await pollCsvStatus(csvId, workspaceId, token);

  // Step 3: Import CSV
  onProgress?.('importing');
  const result = await importCsv(csvId, csvType, workspaceId, token);
