...
<script lang="ts">
    ...
    import { Building2, Download } from '@lucide/svelte';
  ...
  <div class="bg-white px-6 py-3 flex justify-between items-center">
    <div class="flex items-center">
      ...
      <!-- Icon and Heading -->
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-600 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex items-center space-x-4">
      ...
      <div class="relative">
        ...
        >
          <span class="truncate text-slate-900 font-medium">
            {currentView ? getViewName(currentView) : 'Select View'}
          </span>
          <svg 
        ...
        {#if isViewSelectOpen}
          ...
          >
            <div class="max-h-64 overflow-y-auto">
              ...
              {:else}
                {#each views as view (view.id)}
                  <button
                    class="relative flex w-full cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-slate-100 focus:bg-slate-100 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 {currentView?.id === view.id ? 'bg-green-50 text-green-700 font-medium' : 'text-slate-900'}"
                    on:click={() => selectView(view)}
                  >
                    <span class="truncate">{getViewName(view)}</span>
            ...
            <div class="border-t border-slate-200 mt-1 pt-1">
              <button
                class="relative flex w-full cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-slate-100 focus:bg-slate-100 text-green-600"
              ...
              {#each availableFields as field}
                <div class="flex items-center space-x-2 py-1.5">
                  <input
                    type="checkbox"
                    id={`field-${field.id}`}
                    class="h-4 w-4 rounded border-slate-300 text-green-600 focus:ring-green-500 focus:ring-offset-0"
      ...
      <!-- Add Business Button -->
      <button 
        class="inline-flex h-10 items-center justify-center rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white ring-offset-white transition-colors hover:bg-green-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-600 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
