...
<script lang="ts">
  ...
  >
    <div class="p-4 rounded-lg border-l-4 {colors[type]} flex items-start">
      <div class="flex-shrink-0 mr-3 {type === 'success' ? 'text-green-500' : type === 'error' ? 'text-red-500' : type === 'info' ? 'text-blue-500' : 'text-yellow-500'}">
        {@html icons[type]}
      </div>
